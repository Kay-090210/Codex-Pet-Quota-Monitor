...
func (c *Claude) Trigger(ctx context.Context, dryRun bool) (*TriggerResult, error) {
	...
	ptmx, err := startInteractivePTY(ctx, "claude", args...)
	if err != nil {
	...
	if terminal, err := claudeAwait(ctx, ptmx, output, done, claudeStartupTimeout,
	...
	if terminal, err := claudeAwait(ctx, ptmx, output, done, claudeTurnMaxWait,
	...
	case <-ctx.Done():
		return res, claudeInteractiveCancel(ctx, ptmx, done, output)
...
func claudeAwait(ctx context.Context, ptmx interactivePTY, output *limitedBuffer, done <-chan error, maxWait time.Duration, ready func(idle, elapsed time.Duration) bool) (bool, error) {
	start := time.Now()
	...
	for {
		...
		case <-ctx.Done():
			return true, claudeInteractiveCancel(ctx, ptmx, done, output)
		case <-deadline:
			return false, nil
		case <-ticker.C:
...
func claudeInteractiveCancel(ctx context.Context, ptmx interactivePTY, done <-chan error, output *limitedBuffer) error {
	_ = ptmx.Kill()
	_ = ptmx.Close()
	select {
	case <-done:
