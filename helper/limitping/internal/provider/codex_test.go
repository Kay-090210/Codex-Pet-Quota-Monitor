...
func TestCodexInteractiveOutputErrorDetectsWrappedRevokedToken(t *testing.T) {
	output := &limitedBuffer{limit: 4096}
	_, _ = output.Write([]byte("\x1b[31mYour access token could not be refreshed because your refresh token was\x1b[0m\r\n" +
		"revoked. Please log out and sign in again."))

	err := codexInteractiveOutputError(output)
	if err == nil || !strings.Contains(err.Error(), "authentication failed") {
		t.Fatalf("output error = %v, want authentication failure", err)
	}
	// A zero process exit must not hide a turn-level authentication failure.
	if err := codexInteractiveErr(nil, output); err == nil {
		t.Fatal("zero process exit reported success despite authentication failure output")
	}
}

func TestCodexInteractiveOutputErrorIgnoresOrdinaryTUIOutput(t *testing.T) {
	output := &limitedBuffer{limit: 4096}
	_, _ = output.Write([]byte("OpenAI Codex\r\nWorking\r\nOK"))
	if err := codexInteractiveOutputError(output); err != nil {
		t.Fatalf("ordinary output reported as failure: %v", err)
	}
}
