...
const (
	...
	// A fresh isolated CODEX_HOME may need time to initialize its model catalog.
	// Success is taken from the new session's task_complete event, not from TUI
	// silence or process exit, because the TUI stays alive after a failed turn.
	codexTurnMaxWait  = 90 * time.Second
...
func triggerCodex(ctx context.Context, cfg config.ProviderConfig, dryRun bool) (*TriggerResult, error) {
	...
	// Current Codex Desktop engines treat the positional prompt as the initial
	// user turn and submit it after TUI startup. Typing into the composer early
	// can be lost while plugins/MCP servers are still initializing.
	args = append(args, prompt)
	res := &TriggerResult{Command: "codex " + shellJoin(args)}
	if dryRun {
		return res, nil
	}
	turn, err := newCodexSessionObserver(codexSessionHome(), prompt)
	if err != nil {
		return res, fmt.Errorf("preparing Codex turn verification: %w", err)
	}

	ptmx, err := startInteractivePTY(ctx, codexExecutable(), args...)
	if err != nil {
	...
	if terminal, err := codexAwait(ctx, ptmx, output, done, turn, codexTurnMaxWait); terminal || err != nil {
		return res, err
	}

	return res, codexInteractiveStop(ctx, ptmx, done, output)
}

func codexAwait(ctx context.Context, ptmx interactivePTY, output *limitedBuffer, done <-chan error, turn *codexSessionObserver, maxWait time.Duration) (bool, error) {
	...
	for {
		select {
		case err := <-done:
			if complete, turnErr := turn.result(); complete {
				return true, turnErr
			}
			if processErr := codexInteractiveErr(err, output); processErr != nil {
				return true, processErr
			}
			return true, fmt.Errorf("codex interactive exited without a verified assistant response")
		case <-ctx.Done():
			return true, codexInteractiveCancel(ctx, ptmx, done, output)
		case <-deadline:
			if err := codexInteractiveOutputError(output); err != nil {
				_ = ptmx.Kill()
				return true, err
			}
			if complete, err := turn.result(); complete {
				return false, err
			}
			_ = ptmx.Kill()
			return true, fmt.Errorf("codex interactive turn result was not observed within %s", maxWait)
		case <-ticker.C:
			if err := codexInteractiveOutputError(output); err != nil {
				_ = ptmx.Kill()
				return true, err
...
func codexInteractiveStop(ctx context.Context, ptmx interactivePTY, done <-chan error, output *limitedBuffer) error {
	...
	for sent := false; ; {
		...
		select {
		case <-done:
			return codexInteractiveOutputError(output)
		case <-ctx.Done():
			return codexInteractiveCancel(ctx, ptmx, done, output)
		case <-ticker.C:
			_, _ = ptmx.Write([]byte{0x03})
		case <-deadline:
			_ = ptmx.Kill()
...
func codexInteractiveErr(err error, output *limitedBuffer) error {
	if outputErr := codexInteractiveOutputError(output); outputErr != nil {
		return outputErr
	}
	if err == nil {
		return nil
...
// codexInteractiveOutputError recognizes fatal errors rendered inside the TUI.
// The interactive process can stay alive (and later exit successfully) after
// its turn has already failed, so process state alone is not a success signal.
// Normalize ANSI escapes, line wrapping, and box-drawing characters before
// matching the stable authentication wording emitted by Codex.
func codexInteractiveOutputError(output *limitedBuffer) error {
	if output == nil {
		return nil
	}
	plain := claudeNormalizedText(string(output.Bytes()))
	for _, phrase := range []string{
		...
		if strings.Contains(plain, claudeNormalizedText(phrase)) {
			return fmt.Errorf("codex interactive authentication failed; sign in to the Codex profile used by limitping")
		}
	}
	return nil
}

func codexInteractiveCancel(ctx context.Context, ptmx interactivePTY, done <-chan error, output *limitedBuffer) error {
	_ = ptmx.Kill()
	_ = ptmx.Close()
	select {
	case <-done:
